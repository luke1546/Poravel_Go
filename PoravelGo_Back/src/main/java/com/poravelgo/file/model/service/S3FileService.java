...
public class S3FileService implements FileService {
	...
	@Override
	public FileUpload uploadFile(MultipartFile file, String transaction, String roomId) {
		try {
			...
			// uploadDTO 객체 빌드
			FileUpload uploadReq = FileUpload.builder().transaction(transaction).chatRoom(roomId)
					.originFileName(filename).fileDir(key).s3DataUrl(baseUrl + "/" + key).build();
