package com.poravelgo.chat.handler;
...
//            logger.info("Message {}", message.toString());
			...
			case MSG_TYPE_ICE:
				...
				/* 여기도 마찬가지 */
				ChatRoom roomDto = rooms.get(roomId);

				if (roomDto != null) {
					Map<String, WebSocketSession> clients = rtcChatService.getClients(roomDto);

					/*
					 * Map.Entry 는 Map 인터페이스 내부에서 Key, Value 를 쌍으로 다루기 위해 정의된 내부 인터페이스 보통 key 값들을
					 * 가져오는 entrySet() 과 함께 사용한다. entrySet 을 통해서 key 값들을 불러온 후 Map.Entry 를 사용하면서 Key
					 * 에 해당하는 Value 를 쌍으로 가져온다
					 *
...
//                            .orElseThrow(() -> new IOException("Invalid room number received!"));
			...
			case MSG_TYPE_LEAVE:
				...
				// 만약 client 의 값이 존재한다면 - Optional 임으로 isPersent 사용 , null 아니라면 -
				// removeClientByName 을 실행
